...
import com.example.mvvm_wan_kot.common.ext.logW
import com.example.mvvm_wan_kot.common.ext.sharedPreferences
import java.util.*
...
object GlobalUtil {
     ...
     */
    val appName: String
        get() = App.instance.resources.getString(App.instance.applicationInfo.labelRes)
    ...
    val appVersionName: String
        get() = App.instance.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    val appVersionCode: Long
        get() = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            App.instance.packageManager.getPackageInfo(appPackage, 0).longVersionCode
        } else {
            App.instance.packageManager.getPackageInfo(appPackage, 0).versionCode.toLong()
        }
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(App.instance.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
    ...
    fun getDimension(resId: Int): Int {
        return App.instance.resources.getDimensionPixelOffset(resId)
    }
    ...
    fun getResourceId(name: String, type: String): Int {
        return App.instance.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = App.instance.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
    ...
    fun isInstalled(packageName: String): Boolean {
        val packageInfo: PackageInfo? = try {
            App.instance.packageManager.getPackageInfo(packageName, 0)
        } catch (e: PackageManager.NameNotFoundException) {
            null
        }
